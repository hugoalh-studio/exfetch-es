...
Deno.bench("Brotli", () => {
	Brotli.decodeToString(Brotli.encode(sampleContext));
});
Deno.bench("Deflate", () => {
	Deflate.decodeToString(Deflate.encode(sampleContext));
});
Deno.bench("Gzip", () => {
	Gzip.decodeToString(Gzip.encode(sampleContext));
});
Deno.bench("LZ4", () => {
	LZ4.decodeToString(LZ4.encode(sampleContext));
});
Deno.bench("Zlib", () => {
	Zlib.decodeToString(Zlib.encode(sampleContext));
});
